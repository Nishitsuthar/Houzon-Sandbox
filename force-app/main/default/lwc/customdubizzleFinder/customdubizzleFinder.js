...
import fetchLookupData from '@salesforce/apex/DubizzlePropertyFinder.fetchLookupData';
import fetchDefaultRecord from '@salesforce/apex/DubizzlePropertyFinder.fetchDefaultRecord';

const DELAY = 300; // dealy apex callout timing in miliseconds  

export default class CustomdubizzleFinder extends LightningElement {
    
    // public properties with initial default values 
    @api label = 'Dubizzle Building Name';
    @api placeholder = 'search...'; 
    @api iconName = 'standard:account';
    @api sObjectApiName = 'SearchProperties__c';
    @api defaultRecordId = '';
    @api recordId;
    @api dubizzleLocationtext;
   ...
   // initial function to populate default selected lookup record if defaultRecordId provided  
    connectedCallback(){
        ...
        console.log(this.defaultRecordId);
        if(this.defaultRecordId != ''){
            fetchDefaultRecord({recId: this.defaultRecordId})
            .then((result) => {
                ...
                if(result != null && result != undefined && result != ''){
                    ...
                    this.dubizzleLocationtext = this.selectedRecord.Dubizzle_Locationtext__c; 
                    console.log(this.dubizzleLocationtext);          
                    this.handelSelectRecordHelper();
                    const oEvent = new CustomEvent('lookupdubbizle',
                    {
